        ...
        this.runTask = async (parms) => {
            ...
            for (const job of jobs) {
                ...
                try {
                    ...
                    if (err instanceof Error) {
                        jobForError.setError(err.name, err.message, serializeCircular(err.stack));
                    }
                    else {
                        jobForError.setError('UnknownError', serializeCircular(err), '');
                    }
                }
            }
        };
